...
- __isPending__: Indicates whether this update is in a "pending" state. When `true`, that means the update has been downloaded and installed, but the app restart needed to apply it has occurred yet, and therefore, it's changes aren't currently visible to the end-user. *(Boolean)*
