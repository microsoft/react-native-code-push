...
Inside that piece of code, the line ```CodePushReactInstanceManager.builder().setBundleAssetName("index.android.bundle");``` allows CodePush to load your application from the ```index.android.bundle``` file in your ```assets``` folder before any updates are installed. This file can be generated by [running the packager and downloading the bundle](https://gist.github.com/marty-wang/5a71e9d0a6a2c6d6263c). After updates are installed, CodePush will load your app from a writable user directory, where the update has been downloaded.

You will also find the following lines:
...
Those lines load the CodePush configuration properties from the ```CodePush.properties``` file in your ```assets``` folder. Therefore, ensure you have that file in your project. [Here's a sample](/Examples/CodePushDemoApp/android/app/src/main/assets/CodePush.properties).

To summarize, you should have the following files in your project's assets folder, and you should be able to get started!

![Files in Assets](https://cloud.githubusercontent.com/assets/8598682/10624677/c1691ad0-774f-11e5-8402-9927a4b1e6fd.png)
