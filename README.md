...
Inside that piece of code, you will fine the following line:

```java
CodePushReactInstanceManager.builder().setBundleAssetName("index.android.bundle");
``` 

The line allows CodePush to load your application from the ```index.android.bundle``` file in your ```assets``` folder before any updates are installed. This file can be generated by [running the packager and downloading the bundle](https://gist.github.com/marty-wang/5a71e9d0a6a2c6d6263c). After updates are installed, CodePush will load your app from a writable user directory, where the update has been downloaded.
